...
* Clone this repository to a local path on your machine.
  ...
  ```
* Create the `ice_age` conda environment using the `environment.yml` file
  provided in the repository.
  This will install all the required Python packages and dependencies to run the
  application.
...
* Start the application
  ...
  ./start_ice_age.sh
  ```
  When the application is successfully started using the above command, a
  browser window will
  open automatically. The startup page of the application will have instructions
  for importing
  the data downloaded in the previous step. Follow these instructions to finish
  the setup.
...
application is
running. This will stop the Streamlit server and close the application.
